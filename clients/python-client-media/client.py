...
class UserMedia(threading.Thread):
    ...
    def run(self):
        while(True):
            ...
            time.sleep(np.random.uniform(low=0.1, high=0.9))
...
mediaUserThreads = []
for i in range(mediaStreamingUsers[0]):
    t2 = UserMedia()
    t2.start()
    mediaUserThreads.append(t2)
    time.sleep(0.1)
...
while True:
    for days in range(7):
        ...
        for index,rate in enumerate(mediaStreamingUsers):
            for run_time in range(granuality):

                if(index == (len(mediaStreamingUsers) - 1)):
                    a = mediaStreamingUsers[index] + (mediaStreamingUsers[0] - mediaStreamingUsers[index])*(run_time + 1)/granuality
                else :
                    a = mediaStreamingUsers[index] + (mediaStreamingUsers[index + 1] - mediaStreamingUsers[index])*(run_time + 1)/granuality

                number_user = int(a * scale)
                add_number_user = number_user - len(mediaUserThreads)
                # add or remove users
                if(add_number_user > 0):
                    for i in range(add_number_user):
                        t = UserMedia()
                        t.start()
                        mediaUserThreads.append(t)
                        time.sleep(0.1)
                elif(add_number_user < 0):
                    # print("here")
                    for i,t in enumerate(mediaUserThreads):
                        if (i < (add_number_user * (-1))):
                            # print("therehere")
                            t.stop()
                            mediaUserThreads.remove(t)
                            time.sleep(0.1)
                print(len(mediaUserThreads))
                # sleep for 1 min
                time.sleep(20)
