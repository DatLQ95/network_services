...
servingUserThreads = []
for i in range(webServingUsers[0]):
    t = UserServing()
    t.start()
    servingUserThreads.append(t)
    time.sleep(0.01)
...
while True:
    for days in range(7):
        ...
        for index,rate in enumerate(webServingUsers):
            for run_time in range(granuality):

                if(index == (len(webServingUsers) - 1)):
                    a = webServingUsers[index] + (webServingUsers[0] - webServingUsers[index])*(run_time + 1)/granuality
                else :
                    a = webServingUsers[index] + (webServingUsers[index + 1] - webServingUsers[index])*(run_time + 1)/granuality

                number_user = int(a * scale)
                add_number_user = number_user - len(servingUserThreads)
                # add or remove users
                if(add_number_user > 0):
                    ...
                    # print("here")
                    for i,t in enumerate(servingUserThreads):
                        if (i < (add_number_user * (-1))):
                            # print("therehere")
                            t.stop()
                            servingUserThreads.remove(t)
                            time.sleep(0.001)
                print(len(servingUserThreads))
                # sleep for 1 min
                time.sleep(20)
